...
Lx, Ly = 100,100       # simulation box size in Angstroms (arbitrary units)
kT = 0.2                  # thermal energy in eV
p_formation = 0.9         # Probability of vacancy formation/annihilation event per step
...
diffusion_energy_min = 0.1
diffusion_energy_max = 0.15
...
def metropolis_acceptation(delta_E):
    ...
    if delta_E <= 0:
        ...
        # np.random.rand() generates uniform [0,1), accept if less than probability


def attempt_vacancy_formation():
    """Attempt to create a vacancy by removing a random atom"""
    # Use globals for the system state arrays defining atoms, vacancies, and their energies
...
def attempt_vacancy_annihilation():
    ...
    # Globals define system state arrays we modify
...
def attempt_vacancy_diffusion():
    ...
    """
    # Use globals for system state and tracking
