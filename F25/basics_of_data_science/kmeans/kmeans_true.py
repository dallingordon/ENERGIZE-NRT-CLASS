...
class KMeansScratch:
    ...
    def fit(self, X: np.ndarray):
        ...
        for _ in range(self.n_init):
            ...
            for i in range(self.max_iter):
                labels, dists = self._assign_labels(X, centroids)
                inertia = dists.sum()
                if abs(prev_inertia - inertia) <= self.tol * (1 + prev_inertia):
                    break
                prev_inertia = inertia
                centroids = self._recompute_centroids(X, labels, centroids)
    ...
    def _assign_labels(self, X, centroids):
        d2 = self._squared_distances_to_centroids(X, centroids)
