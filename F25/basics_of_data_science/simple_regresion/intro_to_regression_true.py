...
# --------------------------
lin_reg_1 = LinearRegression().fit(X_lin, y_lin)
poly_reg_1 = make_pipeline(PolynomialFeatures(degree=2), LinearRegression()).fit(X_lin, y_lin)
ridge_reg_1 = make_pipeline(PolynomialFeatures(degree=2), Ridge(alpha=1.0)).fit(X_lin, y_lin)

# --------------------------
# Fit models for Dataset 2 (Tanh)
# --------------------------
lin_reg_2 = LinearRegression().fit(X_tanh, y_tanh)
poly_reg_2 = make_pipeline(PolynomialFeatures(degree=25), LinearRegression()).fit(X_tanh, y_tanh)
ridge_reg_2 = make_pipeline(PolynomialFeatures(degree=25), Ridge(alpha=1.0)).fit(X_tanh, y_tanh)
