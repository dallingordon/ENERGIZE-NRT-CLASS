...
def compute_rdf(atoms, r_max, dr, pairs=None):
    ...
    # Prepare pairs for pair-decomposed RDF
    if pairs is None:
        pairs = []
        for i, s1 in enumerate(unique_symbols):
            for s2 in unique_symbols[i:]:
                pairs.append((s1, s2))
    for pair in pairs:
        rdf_pairs[pair] = np.zeros(n_bins)

    # Build symbol indices for quick selection
    indices_dict = {s: np.where(np.array(symbols) == s)[0] for s in unique_symbols}

    # Loop over all atoms to accumulate distances for total RDF
    # Also accumulate for each pair type
    for i in range(n_atoms):
        ...
        for j in range(i + 1, n_atoms):
            ...
            rij_vec = pos_j - pos_i
            if pbc.any():
                # We assume orthogonal cell for simplicity in minimum image
                rij_vec -= np.round(np.linalg.solve(cell.T, rij_vec.T).T).dot(cell)
            dist = np.linalg.norm(rij_vec)
            if dist <= r_max and dist > 1e-8:
                bin_idx = int(dist / dr)
                # Accumulate total histogram (count pairs once)
                hist_total[bin_idx] += 2  # count both atoms

                # Accumulate pair RDF histogram (symmetry)
                pair_key = tuple(sorted((sym_i, sym_j)))
                if pair_key in rdf_pairs:
                    rdf_pairs[pair_key][bin_idx] += 2

    # Normalize histograms to get RDFs
    # For total RDF:
    shell_volumes = 4 / 3 * np.pi * (edges[1:] ** 3 - edges[:-1] ** 3)
    ideal_counts = number_density * shell_volumes * n_atoms
    rdf_total = hist_total / ideal_counts

    # For pair RDFs, normalize by partial number densities
    # partial densities for each species
    n_species = {s: len(indices_dict[s]) for s in unique_symbols}
    densities = {s: n_species[s] / volume for s in unique_symbols}

    for pair in rdf_pairs:
        s1, s2 = pair
        if s1 == s2:
            norm_factor = densities[s1] * shell_volumes * n_species[s1]
        else:
            norm_factor = densities[s2] * shell_volumes * n_species[s1]
        rdf_pairs[pair] /= norm_factor
